...
public class OrganizadorConsola extends MainConsola {
    ...
    private void crearVenue() {
        String id = readNonEmpty("ID venue: ");
        String nombre = readNonEmpty("Nombre: ");
        String ubic = readNonEmpty("Ubicación: ");
        int cap = readInt("Capacidad máx: ", 1, 1_000_000);
        String restr = readNonEmpty("Restricciones: ");
    ...
    private void aprobarVenue() {
        if (db.getVenues().isEmpty()) {
            System.out.println("No hay venues.");
            return;
        }
        String id = readNonEmpty("ID venue: ");
        var v = db.getVenues().stream()
                .filter(x -> x.getId().equals(id))
                .findFirst()
                .orElse(null);

        if (v == null) {
            System.out.println("No existe ese venue.");
            return;
        }
        v.aprobar();
    ...
    private void crearEvento() {
        if (db.getVenues().isEmpty()) {
            System.out.println("No hay venues (cree y apruebe uno).");
            return;
        }

        String id = readNonEmpty("ID evento: ");
        String nombre = readNonEmpty("Nombre: ");
        String sFecha = readNonEmpty("Fecha (YYYY-MM-DD): ");
        String sHora  = readNonEmpty("Hora (HH:MM): ");
        System.out.println("Tipos: 1) musical 2) cultural 3) deportivo");
        int t = readInt("Tipo: ", 1, 3);
        TipoEvento tipo = switch (t) {
            case 1 -> TipoEvento.musical;
            case 2 -> TipoEvento.cultural;
            case 3 -> TipoEvento.deportivo;
        ...
        int idx = readInt("Seleccione venue: ", 1, db.getVenues().size()) - 1;
